...
## 프로젝트 상태 — 개발 보류 (2026-09-17)

**적극적인 개발을 중단하고 현재 구현과 검증 이력을 보존한다. 추가 Phase는 현재 계획하지 않는다.**
최종 구현 범위는 사용자 정의 Phase 7A~7D 자동 공고 발견 MVP이며, 마지막 코드 변경은
`d7b480e`(Streamlit 테스트 SQLite 자원 정리)다. schema v4, 추출기 v2, 수동 등록·로컬 프로필·
설명 가능한 비교, 발견 실행 저장·신규/기존 추적·검토 목록, Streamlit/CLI까지 구현했다.
최종 검증 시점에 자동 테스트 456개가 통과했으며, 이는 실공고 정확도나 실 API 접근 가능성의 증명이 아니다.

주된 차단 요인은 **Work24 일반 채용정보목록·채용정보상세 API의 개인회원 이용 제한**이다.
구현과 자동 검증 이후 실사용 과정에서 개인회원 신청 안내를 확인했다. 채용행사·공채속보·공채기업정보
등과 달리 현재 자동 발견이 의존하는 목록·상세 API는 일반 개인회원이 이용할 수 없다.
따라서 원래 제품 목표인 저장 프로필 기반 자동 공고 발견을 대상 개인 사용자가 사용할 수 없다.
외부 의존성의 실현 가능성 전제가 성립하지 않아 추가 개발의 비용 대비 가치가 낮다고 판단했다.

수동 공고 텍스트 → 요건 추출 → 저장 로컬 프로필 → 설명 가능한 비교는 계속 사용할 수 있다.
다만 수동 중심 도구는 원래 자동 발견 목표와 달라 적극 개발을 이어가지 않는다.
실공고의 복잡한 한국어 표현이 유용한 근거를 `unspecified`로 남기는 recall 한계도 관찰했으나,
이는 보조 품질 한계다. 합성·수작업 fixture 평가는 통과했고 실공고 coverage는 충분히 측정되지 않았다.
구체적인 관찰과 재개 조건은 [README 프로젝트 상태](../README.md#프로젝트-상태--개발-중단--보류)를 따른다.

### 아래 로드맵의 해석

아래 Phase 완료 기록과 이후 항목은 당시의 구현 상태 및 계획을 보존한 **프로젝트 역사**다.
‘다음 작업’, ‘예정’, ‘진행한다’ 등은 현재 활성 계획이 아니다. 과거 API smoke 미실행 기록도 당시 사실이며,
API 제약을 미리 알고 있었다는 의미로 고치지 않는다. 구현 → 테스트 → 실사용 검증 → 외부 제약 확인 →
개발 보류의 순서를 유지한다. 재개 조건이 충족되어 방향을 다시 결정하기 전에는 후속 Phase를 시작하지 않는다.
...
신규10→0→2를 검증했다. additive 이전/복원/실패 rollback, 원자성, source 격리를 확인했다.
다음은 7C 설명 가능한 검토 분류다. 지원 추적·수치 점수·다른 사이트 수집은 범위 밖이다.
...
## Phase 3C 완료 기록 — Requirement Semantics Hardening and Evaluation (2026-09-17)

- `f4997c3`, 깨끗한 작업 트리, 306개 통과에서 시작했다. 감사 후 기존 18개를 유지한 수동
  합성 corpus 60개를 만들고 운영 규칙 변경 전에 v1 지표를 기록했다. 부정 활용형을 추가한
  최종 corpus에도 해당 commit의 추출기를 재실행해 같은 기준 지표를 확인했다.
- v1 사례 일치 40/60·분류 128/144·required FP 3건 → v2 60/60·144/144·13/13그룹,
  required/preferred FP 0건. 정밀도를 우선하며 어려운 기대값을 낮추거나 사례를 삭제하지 않았다.
  [클래스별 fixture 지표](TEST_BASELINE.md#phase-3c-검증-2026-09-17)는 실공고 정확도가 아니다.
- 제한된 공동 나열은 all_of, 대안은 any_of다. 대안의 개별 기술은 unspecified이며 그룹에만
  필수/우대 의미를 둔다. 독립 근거의 우선순위와 약한/반복 근거를 유지한다.
  쉼표 대안·부정 분할과 부정 제목의 상태 누출을 막고, Qualifications/콜론 wrapper를 보강했다.
- extractor version 2, 원문 위치 보존, 네 품질 상태 유지. taxonomy/alias·부모 기술 비추론 유지.
  중첩 논리·복잡한 부정/예외·임의 서식·미지원 기술은 확정하지 않는다.
- 평가 CLI는 분류·그룹·품질 기대값이 모두 맞아야 exit 0이다. 개별/그룹 required·preferred
  오탐도 따로 노출하고, 누락만 늘려 통과할 수 없도록 모든 기대값 일치를 gate로 삼았다.
- 전체 328개 테스트 통과. schema v2·비영속 재처리·원문 보존·수집/실패 refresh·시장 분석·
  추천·UI는 유지한다. 실 Work24 smoke는 수행하지 않았다. 프로필·매칭·Phase 4는 미시작이다.
...
**Phase 3C는 완료했다. Phase 4는 시작하지 않는다.** 후속 요청에서 매칭/표시 범위와 미분류·
상충 근거의 처리 정책을 정하고 any_of 그룹을 반드시 구별해 소비해야 한다. required 빈 목록을 요건 없음으로 해석하거나
