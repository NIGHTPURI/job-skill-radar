...
다음 사용자 지정 단계는 **Phase 1C 데이터 무결성 검토·수정**이다. 아직 시작하지 않았다. 기존 Phase 2 계획과 겹치는 중복 저장 일관성, source/ID 충돌, 공고 없는 기술 행을 우선 다루되, 스키마 변경·migration이 필요한 부분은 별도로 범위를 확정해야 한다. Phase 1B에서 추가 확인한 `미상` 재정제 시 `기타`로 변하는 문제도 함께 검토할 후보이다. Phase 1C 요청 전 기존 Phase 2의 설계를 선제 구현하지 않는다.

데이터 무결성 수정은 동작 변경이므로 현행 재현 테스트와 expected failure를 새 정책에 맞게 명시적으로 갱신해야 한다. FastAPI/React/PostgreSQL 전환은 이 로드맵의 목표가 아니다.
